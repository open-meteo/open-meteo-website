...
{section}_{variable} <span class="token operator">=</span> {#if sdk_type != 'ensemble_api'}<span class="token builtin">next</span><span class="token punctuation">(</span>{/if}<span class="token builtin">filter</span><span class="token punctuation">(</span><span class="token keyword">lambda</span> x<span class="token punctuation">:</span> {@html formatPrismVariableSelector(variable)}<span class="token punctuation">,</span> {section}_series<span class="token punctuation">)</span> {#if sdk_type != 'ensemble_api'}<span class="token punctuation">)</span>{/if}{'\n'}
{/each}
