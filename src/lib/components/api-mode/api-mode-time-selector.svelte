<script lang="ts">
	import { SvelteDate } from 'svelte/reactivity';

	import { fade, fadeOutAbsolute } from '$lib/utils/transitions';

	import { Button } from '$lib/components/ui/button';
	import { Input } from '$lib/components/ui/input';
	import { Label } from '$lib/components/ui/label';
	import * as Select from '$lib/components/ui/select';

	import AnimateHeight from '$lib/components/animate-height/animate-height.svelte';
	import DatePicker from '$lib/components/date/date-picker.svelte';

	import { type Parameters } from '$lib/docs';

	interface Props {
		params: Parameters;
		forecastDaysOptions: { value: string; label: string }[];
		pastDaysOptions: { value: string; label: string }[];
		beginDate: Date;
		lastDate: Date;
	}

	let {
		params = $bindable(),
		forecastDaysOptions,
		pastDaysOptions,
		beginDate,
		lastDate
	}: Props = $props();

	// Forecast mode
	let forecastDays = $derived(forecastDaysOptions.find((fdo) => fdo.value == params.forecast_days));
	let pastDays = $derived(pastDaysOptions.find((pdo) => String(pdo.value) == params.past_days));

	// Historical Forecast mode: archive coverage instead of the forecast window
	const historicalBeginDate = new Date('2016-01-01');
	const historicalLastDate = new Date();

	const historicalEndDefault = (() => {
		const d = new SvelteDate();
		d.setDate(d.getDate() - 2);
		return d.toISOString().split('T')[0];
	})();

	const quickYears = (() => {
		const years = [];
		for (let year = 2022; year <= historicalLastDate.getFullYear(); year++) {
			years.push(year);
		}
		return years;
	})();

	// Single Run mode: most recent 6-hourly runs, one step behind the current
	// interval so the suggested runs are likely disseminated already
	const quickRuns = (() => {
		const runs = [];
		const d = new SvelteDate();
		d.setUTCMinutes(0, 0, 0);
		d.setUTCHours(Math.floor(d.getUTCHours() / 6) * 6 - 6);
		for (let i = 0; i < 3; i++) {
			runs.push(d.toISOString().replace(':00.000Z', ''));
			d.setUTCHours(d.getUTCHours() - 6);
		}
		return runs.reverse();
	})();

	const runLabel = (run: string): string => `${run.slice(5, 10)} ${run.slice(11, 13)}z`;
</script>

<div class="mt-3 -mr-6 flex items-center gap-2 overflow-auto md:mr-0">
	<div class="text-muted-foreground w-22 shrink-0">Time:</div>

	<div class="relative">
		{#if params.api_mode === 'historical_forecast'}
			<div
				in:fade={{ duration: 250, delay: 50 }}
				out:fadeOutAbsolute={{ duration: 200 }}
				class="flex items-center gap-2"
			>
				{#each quickYears as year (year)}
					{@const yearEnd =
						year === historicalLastDate.getFullYear() ? historicalEndDefault : `${year}-12-31`}
					<Button
						variant="outline"
						class="border-primary text-primary hover:bg-primary hover:text-white! dark:text-[#3888ff] {params.start_date ===
							`${year}-01-01` && params.end_date === yearEnd
							? 'bg-primary text-white! dark:text-white!'
							: ''}"
						onclick={() => {
							params.start_date = `${year}-01-01`;
							params.end_date = yearEnd;
						}}>{year}</Button
					>
				{/each}
			</div>
		{:else if params.api_mode === 'single_run'}
			<div
				in:fade={{ duration: 250, delay: 50 }}
				out:fadeOutAbsolute={{ duration: 200 }}
				class="flex items-center gap-2"
			>
				{#each quickRuns as run (run)}
					<Button
						variant="outline"
						class="border-primary text-primary hover:bg-primary hover:text-white! dark:text-[#3888ff] {params.run ===
						run
							? 'bg-primary text-white! dark:text-white!'
							: ''}"
						onclick={() => (params.run = run)}>{runLabel(run)}</Button
					>
				{/each}
			</div>
		{:else}
			<div
				in:fade={{ duration: 250, delay: 50 }}
				out:fadeOutAbsolute={{ duration: 200 }}
				class="border-border flex rounded-md border"
			>
				<Button
					variant="ghost"
					class="items-center gap-1 rounded-e-none opacity-100! duration-300 {params.time_mode ===
					'forecast_days'
						? 'bg-accent cursor-not-allowed'
						: ''}"
					disabled={params.time_mode === 'forecast_days'}
					onclick={() => {
						params.time_mode = 'forecast_days';
						params.start_date = '';
						params.end_date = '';
					}}
				>
					<svg
						class="lucide lucide-clock mr-0.5"
						xmlns="http://www.w3.org/2000/svg"
						width="18"
						height="18"
						viewBox="0 0 24 24"
						fill="none"
						stroke="currentColor"
						stroke-width="2"
						stroke-linecap="round"
						stroke-linejoin="round"
					>
						<circle cx="12" cy="12" r="10" />
						<polyline points="12 6 12 12 16 14" />
					</svg>Forecast Length
				</Button>
				<Button
					variant="ghost"
					class="items-center gap-1 rounded-s-none opacity-100! duration-300 {params.time_mode ===
					'time_interval'
						? 'bg-accent cursor-not-allowed'
						: ''}"
					disabled={params.time_mode === 'time_interval'}
					onclick={() => {
						params.time_mode = 'time_interval';
					}}
				>
					<svg
						class="lucide lucide-calendar-cog mr-0.5"
						xmlns="http://www.w3.org/2000/svg"
						width="18"
						height="18"
						viewBox="0 0 24 24"
						fill="none"
						stroke="currentColor"
						stroke-width="2"
						stroke-linecap="round"
						stroke-linejoin="round"
					>
						<path d="m15.2 16.9-.9-.4" />
						<path d="m15.2 19.1-.9.4" />
						<path d="M16 2v4" />
						<path d="m16.9 15.2-.4-.9" />
						<path d="m16.9 20.8-.4.9" />
						<path d="m19.5 14.3-.4.9" />
						<path d="m19.5 21.7-.4-.9" />
						<path d="M21 10.5V6a2 2 0 0 0-2-2H5a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h6" />
						<path d="m21.7 16.5-.9.4" />
						<path d="m21.7 19.5-.9-.4" />
						<path d="M3 10h18" />
						<path d="M8 2v4" />
						<circle cx="18" cy="18" r="3" />
					</svg>Time Interval
				</Button>
			</div>
		{/if}
	</div>
</div>

<AnimateHeight class="mt-3 md:mt-4">
	{#if params.api_mode === 'historical_forecast'}
		<div in:fade={{ duration: 250, delay: 50 }} out:fadeOutAbsolute={{ duration: 200 }}>
			<DatePicker
				bind:start_date={params.start_date}
				bind:end_date={params.end_date}
				beginDate={historicalBeginDate}
				lastDate={historicalLastDate}
			/>
		</div>
	{:else if params.api_mode === 'single_run'}
		<div
			in:fade={{ duration: 250, delay: 50 }}
			out:fadeOutAbsolute={{ duration: 200 }}
			class="grid gap-3 sm:grid-cols-2 md:gap-6"
		>
			<div class="relative">
				<Input
					id="run"
					type="text"
					class="h-12 cursor-pointer pt-6"
					name="run"
					bind:value={params.run}
				/>
				<Label
					class="text-muted-foreground absolute top-[0.35rem] left-2 z-10 px-1 text-xs"
					for="run">Run (yyyy-mm-ddThh:mm)</Label
				>
			</div>
			<div class="relative">
				<Select.Root name="forecast_days" type="single" bind:value={params.forecast_days}>
					<Select.Trigger
						aria-label="Forecast days input"
						class="h-12 cursor-pointer pt-6 [&_svg]:mb-3">{forecastDays?.label}</Select.Trigger
					>
					<Select.Content preventScroll={false} class="border-border">
						{#each forecastDaysOptions as { value, label } (value)}
							<Select.Item class="cursor-pointer" {value}>{label}</Select.Item>
						{/each}
					</Select.Content>
					<Label class="text-muted-foreground absolute top-[0.35rem] left-2 z-10 px-1 text-xs"
						>Forecast days</Label
					>
				</Select.Root>
			</div>
		</div>
	{:else if params.time_mode === 'time_interval'}
		<div in:fade={{ duration: 250, delay: 50 }} out:fadeOutAbsolute={{ duration: 200 }}>
			<DatePicker
				{beginDate}
				{lastDate}
				bind:start_date={params.start_date}
				bind:end_date={params.end_date}
			/>
		</div>
	{:else}
		<div
			in:fade={{ duration: 250, delay: 50 }}
			out:fadeOutAbsolute={{ duration: 200 }}
			class="grid gap-3 sm:grid-cols-2 md:gap-6"
		>
			<div class="relative">
				<Select.Root name="forecast_days" type="single" bind:value={params.forecast_days}>
					<Select.Trigger id="forecast_days" class="h-12 cursor-pointer pt-6 [&_svg]:mb-3"
						>{forecastDays?.label}</Select.Trigger
					>
					<Select.Content preventScroll={false} class="border-border">
						{#each forecastDaysOptions as { value, label } (value)}
							<Select.Item class="cursor-pointer" {value}>{label}</Select.Item>
						{/each}
					</Select.Content>
					<Label
						for="forecast_days"
						class="text-muted-foreground absolute top-[0.35rem] left-2 z-10 px-1 text-xs"
						>Forecast days</Label
					>
				</Select.Root>
			</div>
			<div class="relative">
				<Select.Root name="past_days" type="single" bind:value={params.past_days}>
					<Select.Trigger id="past_days" class="h-12 cursor-pointer pt-6 [&_svg]:mb-3"
						>{pastDays?.label}</Select.Trigger
					>
					<Select.Content preventScroll={false} class="border-border">
						{#each pastDaysOptions as { value, label } (value)}
							<Select.Item class="cursor-pointer" {value}>{label}</Select.Item>
						{/each}
					</Select.Content>
					<Label
						for="past_days"
						class="text-muted-foreground absolute top-[0.35rem] left-2 z-10 px-1 text-xs"
						>Past days</Label
					>
				</Select.Root>
			</div>
		</div>
	{/if}
</AnimateHeight>
