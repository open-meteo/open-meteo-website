...
export function init() {
    ...
    function previewData(data, downloadTime) {
        ...
        for (var i = 0, len = (data.results || []).length; i < len; i++) {
            ...
            row = $('<tr><td class="p-1"><img height=32 src="/images/country-flags/' + row.country_code.toLowerCase() + '.svg" title="' + row.country + '"/></td><td>' + row.name + '</td><td>' + row.latitude + '</td><td>' + row.longitude + '</td><td>' + row.elevation + '</td><td>' + (row.population||'') + '</td><td>' + (row.admin1||'') + '</td><td>' + (row.admin2||'') + '</td><td>' + (row.admin3||'') + '</td><td>' + (row.admin4||'') + '</td><td>' + row.feature_code + '</td></tr>');
