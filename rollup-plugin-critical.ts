 ...
 * @constructor
 */
function PluginCritical(
	pluginConfig: CriticalPluginConfig,
	callback?: CriticalPluginCallback
): Plugin {
	return {
		name: 'critical',
		async writeBundle(outputOptions, bundle) {
			...
			for (const page of pluginConfig.criticalPages) {
				...
				const criticalTarget =
					pluginConfig.criticalConfig && pluginConfig.criticalConfig.inline == true
						? page.template + '.html'
						: page.template + criticalSuffix;
